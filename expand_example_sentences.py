...
from aqt.utils import showInfo, qconnect, ask_user, getOnlyText
...
def expand_sample_sentences() -> None:
    ...
    if dictionary is None:
        ...
        # 生成prompt
        system_prompt = """根据以下日语单词信息，额外生成包含此单词的例句，用来填充例句库。你需要生成%d条。你的结果应该直接以json格式输出，包含句子、注音句子、简体汉语翻译，给定的单词要用<b></b>标签包围，需要注音的汉字应该与句子其他部分以空格隔开，注意json的数字索引要用""包围使其成为字符串。在句子正确、自然的前提下，尽量在用法、意思上不与已有例句重复，展示词汇全貌。不使用markdown格式，例：{"1": ["あと1時間で着くはずです", "あと1 時間[じかん] で 着[つ] く<b>はず</b>です", "再过一小时应该到了"], "2": ...}"""
        ...
        empty_sentence_n_list = []
        # 生成prompt
        system_prompt = """根据以下日语语法信息，额外生成包含此语法的例句，用来填充例句库。你需要生成%d条。你的结果应该直接以json格式输出，包含句子、简体汉语翻译，句子中应包含假名标注、语法加粗，需要注音的汉字应该与句子其他部分以空格隔开。在句子正确、自然的前提下，尽量在用法、意思、接续规则上不与已有例句重复，展示语法全貌。不使用markdown格式，例：{1: [" 今[こん] 晩[ばん]、 大[おお] 型[がた]の 台[たい] 風[ふう]がこの 地[ち] 方[ほう]へ 近[ちか]づく<b>恐[おそ]</b><b>れがあります</b>。", "今晚可能有强台风靠近该地区。"], 2: ...}"""
...
def expand_sample_sentences_manual() -> None:
    """
    填充示例句子字段。
    对于每个卡片，都会有四个例句的字段簇：
        - SentTypeN
    ...
    if dictionary is None:
        ...
        # 生成prompt
        system_prompt = """
你是一个日语学习人工智能助手。根据以下的日语单词信息，额外生成包含此单词的例句，用来填充例句库。
你需要生成%d条。你的结果应该直接以json格式输出，包含句子、注音句子、简体汉语翻译，给定的单词要用<b></b>标签包围，需要注音的汉字应该与句子其他部分以空格隔开，注意json的数字索引要用""包围使其成为字符串。
你生成的句子应当在语法上正确、规范；在词语的意思上准确自然，符合语境，并能体现出这个词的细微语感
（例如，你应当知道「意見を受け取る」是「ただ単に意見をもらうイメージ」，「意見を受け止める」是「間違いなどを指摘された場合、その指摘を受け入れ、更に考えるイメージです」，「意見を受け入れる」是「相反する意見だが、了解する」。你要体察这些细小差异，所生成的例句需要准确且自然）。
你的句子应当尽量在用法、意思上不与已有例句重复，从而展示词汇全貌。
直接输出结果，不使用markdown格式输出。
输出例：{"1": ["あと1時間で着くはずです", "あと1 時間[じかん] で 着[つ] く<b>はず</b>です", "再过一小时应该到了"], "2": ...}"""
        ...
        prompt = system_prompt % len(empty_sentence_n_list) + "\n" + prompt

        QApplication.clipboard().setText(prompt)
        tooltip("提示词已复制到剪贴板！")

        content = getOnlyText("输入AI生成的结果\n提交空白内容以撤销操作", title="手动填充AI例句", default="", parent=None)

        if not content.strip():
            tooltip("Operation cancelled by user.")
            return
        
        ai_generated_sentences: list = list(json.loads(content).values())
        ...
        empty_sentence_n_list = []
        # 生成prompt
        system_prompt = """你是一个日语学习人工智能助手。根据以下日语语法信息，额外生成包含此语法的例句，用来填充例句库。你需要生成%d条。你的结果应该直接以json格式输出，包含句子、简体汉语翻译，句子中应包含假名标注、语法加粗，需要注音的汉字应该与句子其他部分以空格隔开。在句子正确、自然的前提下，尽量在用法、意思、接续规则上不与已有例句重复，展示语法全貌。不使用markdown格式，例：{1: [" 今[こん] 晩[ばん]、 大[おお] 型[がた]の 台[たい] 風[ふう]がこの 地[ち] 方[ほう]へ 近[ちか]づく<b>恐[おそ]</b><b>れがあります</b>。", "今晚可能有强台风靠近该地区。"], 2: ...}"""
        ...
        prompt = system_prompt % number_of_sentence_to_generate + "\n" + prompt
