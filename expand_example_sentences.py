...
def expand_sample_sentences() -> None:
    ...
    if dictionary is None:
        ...
        # 生成prompt
        system_prompt = """根据以下日语单词信息，额外生成包含此单词的例句，用来填充例句库。你需要生成%d条。你的结果应该直接以json格式输出，包含句子、注音句子、简体汉语翻译，需要注音的汉字应该与句子其他部分以空格隔开，不使用markdown格式，例：{1: ["あと1時間で着くはずです", "あと1 時間[じかん]で 着[つ]く<b>はず</b>です", "再过一小时应该到了"], 2: ...}"""
        ...
        empty_sentence_n_list = []
        # 生成prompt
        system_prompt = """根据以下日语语法信息，额外生成包含此语法的例句，用来填充例句库。你需要生成%d条。你的结果应该直接以json格式输出，包含句子、简体汉语翻译，句子中应包含假名标注、语法加粗，需要注音的汉字应该与句子其他部分以空格隔开，不使用markdown格式，例：{1: [" 今[こん] 晩[ばん]、 大[おお] 型[がた]の 台[たい] 風[ふう]がこの 地[ち] 方[ほう]へ 近[ちか]づく<b>恐[おそ]</b><b>れがあります</b>。", "今晚可能有强台风靠近该地区。"], 2: ...}"""
