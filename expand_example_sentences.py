...
def expand_sample_sentences() -> None:
    ...
    if dictionary is None:
        ...
        # 生成prompt
        system_prompt = """根据以下日语单词信息，额外生成包含此单词的例句，用来填充例句库。你需要生成%d条。你的结果应该直接以json格式输出，包含句子、注音句子、简体汉语翻译，给定的单词要用<b></b>标签包围，需要注音的汉字应该与句子其他部分以空格隔开。在句子正确、自然的前提下，尽量在用法、意思上不与已有例句重复，展示词汇全貌。不使用markdown格式，例：{1: ["あと1時間で着くはずです", "あと1 時間[じかん] で 着[つ] く<b>はず</b>です", "再过一小时应该到了"], 2: ...}"""
